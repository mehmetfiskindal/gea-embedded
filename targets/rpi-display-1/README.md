...
## Hızlı başlangıç (Hibrit Yöntem - Pi Zero için Önerilen)

Standard cross-derleyiciler (`gcc-arm-linux-gnueabihf`) ARMv7 başlangıç kütüphaneleri getirdiğinden Pi Zero'da `Illegal instruction` hatasını engellemek için **Vite derlemesini Host'ta, C derlemesini ise Pi'de** yapıyoruz:

```bash
# 1) Pi OS Lite SD kart yapılandırması (Bir kez):
./targets/rpi-display-1/scripts/install-zero.sh pi@raspberrypi.local

# 2) Host'ta Vite build işlemi ve Pi'ye senkronizasyon:
cd examples/tic-tac-toe && GEA_EMBEDDED_TARGET=rpi npm run build && cd ../..
./targets/rpi-display-1/scripts/geat-rpi.sh sync pi@raspberrypi.local --with-apps

# 3) Pi üzerinde C derlemesi (SSH ile bağlanıp):
ssh pi@raspberrypi.local 'cd ~/gea-embedded && ./targets/rpi-display-1/scripts/geat-rpi.sh build --app=tic-tac-toe --skip-vite'

# 4) Pi üzerinde ikili dosyayı kurup çalıştırma (SSH ile):
ssh pi@raspberrypi.local 'sudo cp ~/gea-embedded/build/rpi/geat-app-tic-tac-toe /opt/gea-embedded/apps/tic-tac-toe/geat-app && GEA_RPI_APP_ID=tic-tac-toe /opt/gea-embedded/apps/tic-tac-toe/geat-app'
```
